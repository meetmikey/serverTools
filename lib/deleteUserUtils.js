        ...
        async.each( foundAttachments, function( attachment, eachCallback ) {
          ...
          deleteUserUtils.checkAndDeleteAttachmentInfo( attachment, function(err) {
            if ( err ) {
              ...
              esUtils.delete( esUtils.getIndexAliasForUser (attachment.userId), 'document', String(attachmentId), null, function( esUtilsError ) {
                if ( esUtilsError ) {
                  eachCallback( winston.makeError('esUtilsError', {esUtilsError: esUtilsError}));
                } else {
                  attachment.remove (function (err) {
                    if (err) {
                      eachCallback (winston.makeMongoError (err));
                    } else {
                      eachCallback();
                    }
                  });
        ...
        async.each( foundLinks, function( link, eachCallback ) {
          ...
          deleteUserUtils.checkAndDeleteLinkInfo (link, function (err) {
            if (err) {
              ...
              esUtils.delete( esUtils.getIndexAliasForUser (user._id), 'document', String(linkId), null, function( esUtilsError ) {
                if ( esUtilsError ) {
                  eachCallback( winston.makeError('esUtilsError', {esUtilsError: esUtilsError}));
