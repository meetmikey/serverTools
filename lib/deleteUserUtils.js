            ...
            if ( mongoErr ) {
              ...
              deleteUserUtils.checkAndDeleteAttachmentInfo( attachment, function(err) {
                if ( err ) {
                  ...
                  esUtils.delete( esConnect.indexName, 'document', String(attachmentId), null, function( esUtilsError ) {
                    if ( esUtilsError ) {
                      eachCallback( winston.makeError('esUtilsError', {esUtilsError: esUtilsError}));
                    } else {
                      console.log ('delete from index');
                      eachCallback();
                    }
                  });
                }
...
exports.checkAndDeleteAttachmentInfo = function( attachment, callback ) {
  ...
  AttachmentModel.findOne({hash: attachment.hash, fileSize: attachment.fileSize}, function(mongoErr, foundAttachment) {
    if ( mongoErr ) {
...
exports.deleteAttachmentInfo = function( attachment, callback ) {
  ...
  if ( ! attachment ) { callback( winston.makeMissingParamError('attachment') ); return; }
  if ( ! attachment.hash ) { callback( winston.makeMissingParamError('attachment.hash') ); return; }
  if ( ! attachment.fileSize ) { callback( winston.makeMissingParamError('attachment.fileSize') ); return; }
  ...
  AttachmentInfoModel.findOneAndRemove( filter, function( mongoErr ) {
    if ( mongoErr ) {
      callback( winston.makeMongoError( mongoErr ) );
            ...
            if ( mongoErr ) {
              ...
              deleteUserUtils.checkAndDeleteLinkInfo (link, function (err) {
                if (err) {
                  ...
                  esUtils.delete( esConnect.indexName, 'document', String(linkId), String(link.comparableURLHash), function( esUtilsError ) {
                    if ( esUtilsError ) {
                      eachCallback( winston.makeError('esUtilsError', {esUtilsError: esUtilsError}));
...
exports.checkAndDeleteLinkInfo = function( link, callback ) {
  console.log ('checkAndDeleteLinkInfo')

  if ( ! link ) { callback( winston.makeMissingParamError('link') ); return; }

  LinkModel.findOne({comparableURLHash: link.comparableURLHash}, function(mongoErr, foundLink) {
    if ( mongoErr ) {
      callback( winston.makeMongoError( mongoErr ) );
...
exports.deleteLinkInfo = function( link, callback ) {
  ...
  LinkInfoModel.findOneAndRemove( filter, function( mongoErr ) {
    if ( mongoErr ) {
      callback( winston.makeMongoError( mongoErr ) );

    } else {
      var path = cloudStorageUtils.getLinkInfoPath( link );
      var inAzure = false;
      cloudStorageUtils.deleteFile( path, inAzure, function(err) {
        if ( err ) {
          callback( err );
