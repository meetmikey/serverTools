  ...
  UserModel.findOne( {email: userEmail}, function( err, foundUser ) {
    if ( err ) {
      ...
      var actions = [
        ...
        , function( seriesCallback ) { deleteUserUtils.deleteAttachmentsByUserAndShardKey( foundUser, shardKey, seriesCallback ); }
...
exports.deleteAttachmentsByUserAndShardKey = function( user, shardKey, callback ) {
  ...
  var filter = {userId: user._id, shardKey: shardKey};
