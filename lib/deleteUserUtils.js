...
exports.performUserDelete = function( userEmail, shouldDeleteUserObject, callback ) {
  ...
  UserModel.findOne( {email: userEmail}, function( err, foundUser ) {
    if ( err ) {
      ...
      var deletedUser = deleteUserUtils.createDeletedUserFromUser( foundUser, shouldDeleteUserObject );
      ...
      deletedUser.save( function( mongoErr ) {
        if ( mongoErr ) {
          ...
          deleteUserUtils.performUserDataDelete( foundUser, shouldDeleteUserObject, function(err) {
            if ( err ) {
              callback( err );
...
exports.performUserDataDeleteByUserId = function( userId, callback ) {
  ...
  deleteUserUtils.performUserDataDelete( partialUser, false, callback );
}

exports.performUserDataDelete = function( user, shouldDeleteUserObject, callback ) {

  if ( ! user ) { callback( winston.makeMissingParamError('user') ); return; }
  if ( ! user._id ) { callback( winston.makeMissingParamError('user._id') ); return; }

  var actions = [
      function( seriesCallback ) { deleteUserUtils.deleteUserOnboardingStates( user, seriesCallback ); }
    , function( seriesCallback ) { deleteUserUtils.deleteAttachmentsByUser( user, seriesCallback ); }
    , function( seriesCallback ) { deleteUserUtils.deleteLinks( user, seriesCallback ); }
    , function( seriesCallback ) { deleteUserUtils.deleteActiveConnections( user, seriesCallback ); }
    , function( seriesCallback ) { deleteUserUtils.deleteMailboxes( user, seriesCallback ); }
    , function( seriesCallback ) { deleteUserUtils.deleteMails( user, seriesCallback ); }
    , function( seriesCallback ) { deleteUserUtils.deleteReceiveMRs( user, seriesCallback ); }
    , function( seriesCallback ) { deleteUserUtils.deleteResumeDownloadStates( user, seriesCallback ); }
    , function( seriesCallback ) { deleteUserUtils.deleteBandwiths ( user, seriesCallback ); }
    , function( seriesCallback ) { deleteUserUtils.deleteSentAndCoReceiveMRs( user, seriesCallback ); }
  ];

  if ( shouldDeleteUserObject ) {
    actions.push( function( seriesCallback ) { deleteUserUtils.deleteUser( user, seriesCallback ); } );
  }
