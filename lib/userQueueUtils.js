  ...
  query.exec (function (err, states) {
    ...
    async.eachSeries (states, function (state, cb) {
      UserModel.findById (state.userId, function (err, user) {
        ...
        userQueueUtils.createResumeDownloadNowSpecial( user, lastCompleted, true, function(err) {
          if ( err ) {
            cb (err);

          } else {
            winston.doInfo('done with user', {email: user.email});
            cb();
          }
        });
...
exports.createResumeDownloadNowSpecial = function( user, lastCompleted, forceCreation, callback ) {
  console.log ('createResumeDownloadNowSpecial')
